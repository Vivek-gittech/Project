...
const CustomerDashboard = () => {
  ...
  // --- Auth Check & Session Logic ---
  useEffect(() => {
    const storename = localStorage.getItem("customer");
    if (storename) {
      setCustomer(storename);
    } else {
      // If someone tries to access dashboard without being logged in
      navigate("/login");
    }
  }, [navigate]);
        ...
        >
          ...
          <div className="flex items-center gap-10">
            <nav className="hidden lg:flex items-center gap-8">
              ...
              <Link to="/History" className="text-[11px] font-black text-zinc-500 hover:text-[#135bec] transition-all uppercase tracking-[0.15em]">History</Link>
            </nav>
            
            <button 
              onClick={handleLogout}
              className="px-6 py-2 bg-zinc-900 dark:bg-zinc-100 dark:text-zinc-900 text-white rounded-full text-[10px] font-black uppercase tracking-widest transition-transform hover:scale-105 active:scale-95"
            >
              Logout
            </button>
          </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <AnimatePresence>
              {activeOrder && (
                <motion.div 
                  ...
                  className="bg-white dark:bg-[#161e31] rounded-[2.5rem] p-10 shadow-[0_20px_50px_rgba(0,0,0,0.04)] border border-zinc-100 dark:border-zinc-800 relative overflow-hidden"
                >
                  <div className="relative z-10">
                    ...
                    <div className="relative h-4 w-full bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden mb-6">
                      <motion.div 
                        ...
                        className="h-full bg-gradient-to-r from-[#135bec] to-[#3b82f6] rounded-full" 
                      />
                    </div>
                    <div className="flex items-center gap-3 text-zinc-400 font-bold text-sm">
                      <span className="material-symbols-outlined text-lg animate-pulse">
                        {activeOrder.status?.toLowerCase() === 'ready' ? 'check_circle' : 'timer'}
                      </span>
          ...
          <div className="space-y-10">
             <motion.div variants={itemVariants} className="bg-zinc-900 p-10 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                  ...
                  <div className="space-y-3">
                    ...
                    <button className="flex items-center justify-between w-full p-4 bg-white/5 rounded-2xl hover:bg-white/10 transition-all font-bold text-sm">
                      Help Support <span className="material-symbols-outlined text-lg">contact_support</span>
                    </button>
                  </div>
                </div>
             ...
             <motion.div variants={itemVariants} className="bg-[#135bec] p-8 rounded-[2.5rem] text-white">
                <span className="material-symbols-outlined text-3xl mb-4">stars</span>
                <h3 className="text-xl font-black mb-2">Loyalty Points</h3>
                <p className="text-blue-100 text-sm font-medium mb-4">You have 250 points! You're only 50 points away from a free Latte.</p>
